...
int32_t main(){
    ...
    looper(i,0,n){
        if(i==0) prefix_max[i]=arr[i];
        else prefix_max[i] = max(prefix_max[i-1],arr[i]);
    }
    ...
    int ans=0;
    looper(i,0,n-1){
        if(prefix_max[i]<= suffix_min[i+1]) ans++;
    }
    cout<<ans+1; //+1 coz we have to make one grp for last element for sure
    return 0;
}