// this question will also give us a hint how can we calculate middle element if we want 
#include <iostream>
#include <cstdio>
#include <cstdlib>
#include <algorithm>
...
class Linked_List{
    public:
        ...
        double medianForSortedLL(){
            if(!head) return -1; //in case our LL is empty 
            ...
            if(fast=NULL){
                return (prevSlow->data + slow->data)/2.0; // to return double value
            }else{
                return slow->data;
            }
        }
...
// doing the ques assuming our LL is singly linked 
// in case of doubly linked LL, no need to make the prev_slow pointer
int32_t main(){
    Linked_List l;
    l.pb(1);
    l.pb(2);
