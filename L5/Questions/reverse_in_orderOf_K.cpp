...
class Linked_List{
    public:
        ...
        node* reverse_SinglyLL_Korder(node *currHead,int k){
            if(currHead==NULL) return NULL;
            node *curr=currHead,*prev=NULL;
            int count=0;
            ...
            //now curr is pointing towards (k+1)th node
            currHead->next = reverse_SinglyLL_Korder(curr,k);
            return prev;
        }
...
int32_t main(){
    ...
    node *newhead = l.reverse_SinglyLL_Korder(l.head,3);
    l.head= newhead;
    l.print_iteratively();
    return 0;
}